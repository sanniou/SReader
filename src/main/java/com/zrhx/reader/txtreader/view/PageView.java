 ...
 */
public class PageView extends ReadView {
    private static final String TAG = "PageView";
    private int mCornerX = 0; // 拖拽点对应的页脚
    private int mCornerY = 0;
    private Path mPath0;
    private Path mPath1;
    Bitmap mCurPageBitmap; // 当前页
    ...
    public boolean doTouchEvent(MotionEvent event) {
        if (event.getAction() == MotionEvent.ACTION_MOVE) {
            //防止手势越界，-1以便于计算
            mTouch.x = Math.min(event.getX(), mWidth - 1);
            mTouch.y = Math.min(event.getY(), mHeight - 1);
            this.postInvalidate();
        }
